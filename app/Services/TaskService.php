...
final class TaskService
{
    ...
    public function generateFromNewPost(array $payload): array
    {
        ...
        $scenarios = App::scenarios()->matching($artistId, 'NEW_POST');
        $created = [];
        foreach ($scenarios as $scenario) {
            $accountId = (string) $scenario['account_id'];
            if ($this->existsForPostAccount($postId, $accountId)) {
                continue;
            }
            $delay = scenario_delay_seconds($scenario['timing'] ?? ['type' => 'fixed', 'delay_seconds' => 0]);
            ...
            $task = $this->storage->insert('tasks', [
                ...
                'last_error' => null,
                'error_code' => null,
                'steps' => $scenario['steps'] ?? [['type' => 'OPEN_POST']],
            ]);
            $created[] = $task;
    ...
    public function insert(array $task): array
    {
        $now = now_utc();
        $task['id'] = $task['id'] ?? generate_id('task');
        $task['created_at'] = $task['created_at'] ?? $now;
        $task['status'] = $task['status'] ?? 'pending';
        $task['attempts'] = $task['attempts'] ?? 0;
        return $this->storage->insert('tasks', $task);
    }
    ...
    public function supervision(): array
    {
        ...
        foreach ($this->all() as $task) {
            ...
            $artist = $post ? ($artists[$post['artist_id'] ?? ''] ?? null) : ($artists[$task['artist_id'] ?? ''] ?? null);
            $scheduled = (string) ($task['scheduled_at'] ?? '');
            $eta = null;
            if (in_array($task['status'] ?? '', ['pending', 'ready', 'queued'], true) && $scheduled !== '') {
    ...
    public function counts(): array
    {
        ...
        foreach ($items as $task) {
            ...
            if (in_array($status, ['pending', 'ready', 'queued'], true)) {
